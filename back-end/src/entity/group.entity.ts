...
export class Group {
  ...
  public prepareToCreate(input: CreateGroupInput) {
    ...
    this.number_of_weeks = input.number_of_weeks ? input.number_of_weeks : null
    this.roll_states = input.roll_states ? input.roll_states : null
    this.incidents = input.incidents ? input.incidents : null
    this.ltmt = input.ltmt ? input.ltmt : null
